...
#xlsx = input("Digite o nome do arquivo xlsx:")
xlsx = "teste.xlsx"
workbook = xlsxwriter.Workbook(xlsx)
worksheet = workbook.add_worksheet()

worksheet.write(0 , 0, 'Nome:')
worksheet.write(0 , 1, 'Categoria:')
worksheet.write(0 , 2, 'Preço atual:')
worksheet.write(0 , 3, 'Preço cheio')
worksheet.write(0 , 4, 'Desconto')
worksheet.write(0 , 5, 'Consultado em:')
worksheet.write(0 , 6, 'Site:')
worksheet.write(0 , 7, 'Disponibilidade:')

def track(url, z):
    res = requests.get(url, headers={'user-agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.90 Safari/537.36'}) # Trocar user-agent pelo da sua máquina
	...
	print()
        ...
        worksheet.write(z, 0, nome) 
        worksheet.write(z, 1, cat) 
        worksheet.write(z, 2, price) 
        worksheet.write(z, 3, fup)
        worksheet.write(z, 4, desconto)
        worksheet.write(z, 5, data_em_texto)
        worksheet.write(z, 6, dominio)
        worksheet.write(z, 7, disp)
